...
import { ClipboardList, LayoutDashboard, Clock, PlayCircle, Loader2, CheckCircle2 } from 'lucide-react';
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="bg-surface border border-border rounded-3xl p-6 shadow-sm">
          <h2 className="text-lg font-bold text-main flex items-center gap-2 mb-6">
            ...
            {t.onboarding.dashboard?.queueSection || 'Queue — Awaiting VSM Assignment'}
          </h2>
          {queueTickets.length === 0 ? (
            <div className="text-center py-8 text-muted text-sm border border-dashed border-border rounded-xl">
              {t.onboarding.dashboard?.noQueue || 'No tickets in queue'}
            </div>
          ) : (
              ...
              {queueTickets.map(ticket => (
                <div key={ticket.id} className="flex flex-col sm:flex-row sm:items-center justify-between p-4 bg-surface-hover/50 border border-border rounded-xl gap-4">
                  ...
                  <button 
                    ...
                    className="px-4 py-2 bg-brand/10 text-brand text-sm font-bold rounded-lg hover:bg-brand hover:text-white transition-colors"
                  >
                    {t.onboarding.actions.assignVsm}
                  </button>
                </div>
              ))}
            </div>
